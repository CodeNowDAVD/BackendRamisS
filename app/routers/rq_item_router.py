...
from app.schemas.rq_item_schema import RQItemCreate, RQItemResponse

router = APIRouter(prefix="/rq-items", tags=["RQ Items"])

@router.post("/{rq_id}", response_model=RQItemResponse)
def create_item(rq_id: int, item: RQItemCreate, db: Session = Depends(get_db)):
    service = RQItemService(db)
    return service.create_item(item.dict(), rq_id)

@router.get("/{rq_id}", response_model=List[RQItemResponse])
def list_items(rq_id: int, db: Session = Depends(get_db)):
    service = RQItemService(db)
    return service.get_items_by_rq(rq_id)

@router.put("/{item_id}/estado")
def update_item_estado(item_id: int, estado: str, db: Session = Depends(get_db)):
    service = RQItemService(db)
